...
ColumnVector Gibbs_sampler_for_Multivariate_Gaussian(int index, ColumnVector Previous_value, SymmetricMatrix C, ColumnVector mean)
{
    ...
    double sigma11 = C(1, 1);
    double sigma12 = C(1, 2);
    double sigma21 = C(2, 1);
    double sigma22 = C(2, 2);
    //ColumnVector mean(mean.nrows());
    //ColumnVector Previous_value(mean.nrows());
    ...
    if (index == 1)
    {
        double mu12 = mean(1) + sigma12 * (1 / sigma22) * (Previous_value(2) - mean(2));
        double sd12 = sigma11 - sigma21 * (1 / sigma22) * sigma21;
        x = get_gaussian(mu12, sd12);
    }
    if (index == 2)
    {
        double mu21 = mean(2) + sigma21 * (1 / sigma11) * (Previous_value(1) - mean(1));
        double sd21 = sigma22 - sigma12 * (1 / sigma11) * sigma12;
        x = get_gaussian(mu21, sd21);
    }
    xx(index) = x;
...
{
    return (1/sqrt(pow((2*PI),2)*C.Determinant()))*exp((-0.5*(x-mean).t()*C.i()*(x-mean)).as_scalar());
}
